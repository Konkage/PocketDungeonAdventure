...
public class InputMode_MeleeBattleDebug : I_InputMode
{
    ...
    public void UpdateInputMode(UnitPlayer _Player, ControllerPlayer _Controller)
    {
        ...
        {
            m_HasStartedSlash = true;
            m_SlashStartPosition = new Vector2(Input.mousePosition.x, Input.mousePosition.y);
        }
        if (Input.GetKeyUp(KeyCode.Mouse0))
        {
            m_HasStartedSlash = false;
            if (UserInterface.IsMeleeSlashHitting(m_SlashStartPosition, new Vector2(Input.mousePosition.x, Input.mousePosition.y)))
            {
                _Player.AttackEnemy();
            }
            UserInterface.HideSlashLine();
        ...
        if (m_HasStartedSlash)
        {
            UserInterface.DisplaySlashLine(m_SlashStartPosition, new Vector2(Input.mousePosition.x, Input.mousePosition.y));
        }
    }
